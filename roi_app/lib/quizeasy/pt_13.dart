...
class QuizScreen13 extends StatefulWidget {
  const QuizScreen13({super.key});

  @override
  State<QuizScreen13> createState() => _QuizScreen13State();
}
...
class _QuizScreen13State extends State<QuizScreen13> {
  ...
  Future<List<QuizQuestionData>> _fetchQuestions() async {
    try {
      QuerySnapshot snapshot = await _firestore.collection('pt_13e').get();
...
class _QuizLogicWidgetState extends State<QuizLogicWidget> {
  ...
  void _showFeedbackDialog(bool isCorrect) {
    showDialog(
      ...
      builder: (context) => _gamifiedDialog(
        ...
        content: isCorrect ? "Brilliant! You're a natural! 🚀" : "The correct answer is:\n${widget.questions[_currentIndex].correctAnswer}",
  ...
  Widget build(BuildContext context) {
    return ThemedQuizScreen(
      title: 'Part XIII: Trade & Commerce Quiz',
