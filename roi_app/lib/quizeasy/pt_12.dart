...
class QuizScreen12 extends StatefulWidget {
  const QuizScreen12({super.key});

  @override
  State<QuizScreen12> createState() => _QuizScreen12State();
}
...
class _QuizScreen12State extends State<QuizScreen12> {
  ...
  Future<List<QuizQuestionData>> _fetchQuestions() async {
    try {
      QuerySnapshot snapshot = await _firestore.collection('pt_12e').get();
...
class _QuizLogicWidgetState extends State<QuizLogicWidget> {
  ...
  void _showFeedbackDialog(bool isCorrect) {
    showDialog(
      ...
      builder: (context) => _gamifiedDialog(
        ...
        content: isCorrect ? "Magical! You're unstoppable! 🚀" : "The correct answer is:\n${widget.questions[_currentIndex].correctAnswer}",
  ...
  Widget build(BuildContext context) {
    return ThemedQuizScreen(
      title: 'Part XII: Finance & Contract Quiz',
