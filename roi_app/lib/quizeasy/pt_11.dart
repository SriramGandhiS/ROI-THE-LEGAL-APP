...
class QuizScreen11 extends StatefulWidget {
  const QuizScreen11({super.key});

  @override
  State<QuizScreen11> createState() => _QuizScreen11State();
}
...
class _QuizScreen11State extends State<QuizScreen11> {
  ...
  Future<List<QuizQuestionData>> _fetchQuestions() async {
    try {
      QuerySnapshot snapshot = await _firestore.collection('pt_11e').get();
...
class _QuizLogicWidgetState extends State<QuizLogicWidget> {
  ...
  void _showFeedbackDialog(bool isCorrect) {
    showDialog(
      ...
      builder: (context) => _gamifiedDialog(
        ...
        content: isCorrect ? "Stellar work! 🚀" : "The correct answer is:\n${widget.questions[_currentIndex].correctAnswer}",
  ...
  Widget build(BuildContext context) {
    return ThemedQuizScreen(
      title: 'Part XI: The States (Cont.) Quiz',
