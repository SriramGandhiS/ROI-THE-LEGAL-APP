...
class QuizScreen17 extends StatefulWidget {
  const QuizScreen17({super.key});

  @override
  State<QuizScreen17> createState() => _QuizScreen17State();
}
...
class _QuizScreen17State extends State<QuizScreen17> {
  ...
  Future<List<QuizQuestionData>> _fetchQuestions() async {
    try {
      QuerySnapshot snapshot = await _firestore.collection('pt_17e').get();
...
class _QuizLogicWidgetState extends State<QuizLogicWidget> {
  ...
  void _showFeedbackDialog(bool isCorrect) {
    showDialog(
      ...
      builder: (context) => _gamifiedDialog(
        ...
        content: isCorrect ? "Outstanding! Keep it up! 🚀" : "The correct answer is:\n${widget.questions[_currentIndex].correctAnswer}",
  ...
  Widget build(BuildContext context) {
    return ThemedQuizScreen(
      title: 'Part XVII: Official Language Quiz',
