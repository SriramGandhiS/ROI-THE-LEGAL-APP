...
class QuizScreen15 extends StatefulWidget {
  const QuizScreen15({super.key});

  @override
  State<QuizScreen15> createState() => _QuizScreen15State();
}
...
class _QuizScreen15State extends State<QuizScreen15> {
  ...
  Future<List<QuizQuestionData>> _fetchQuestions() async {
    try {
      QuerySnapshot snapshot = await _firestore.collection('pt_15e').get();
  ...
  void _showFeedbackDialog(bool isCorrect) {
    showDialog(
      ...
      builder: (context) => _gamifiedDialog(
        ...
        content: isCorrect ? "You're a star! 🌟🚀" : "The correct answer is:\n${widget.questions[_currentIndex].correctAnswer}",
