...
class QuizScreen18 extends StatefulWidget {
  const QuizScreen18({super.key});

  @override
  State<QuizScreen18> createState() => _QuizScreen18State();
}
...
class _QuizScreen18State extends State<QuizScreen18> {
  ...
  Future<List<QuizQuestionData>> _fetchQuestions() async {
    try {
      QuerySnapshot snapshot = await _firestore.collection('pt_18e').get();
...
class _QuizLogicWidgetState extends State<QuizLogicWidget> {
  ...
  void _showFeedbackDialog(bool isCorrect) {
    showDialog(
      ...
      builder: (context) => _gamifiedDialog(
        ...
        content: isCorrect ? "Supercharged learning! 🚀" : "The correct answer is:\n${widget.questions[_currentIndex].correctAnswer}",
  ...
  Widget build(BuildContext context) {
    return ThemedQuizScreen(
      title: 'Part XVIII: Emergency Provisions Quiz',
