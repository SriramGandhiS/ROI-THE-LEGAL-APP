...
class QuizScreen14 extends StatefulWidget {
  const QuizScreen14({super.key});

  @override
  State<QuizScreen14> createState() => _QuizScreen14State();
}
...
class _QuizScreen14State extends State<QuizScreen14> {
  ...
  Future<List<QuizQuestionData>> _fetchQuestions() async {
    try {
      QuerySnapshot snapshot = await _firestore.collection('pt_14e').get();
...
class _QuizLogicWidgetState extends State<QuizLogicWidget> {
  ...
  void _showFeedbackDialog(bool isCorrect) {
    showDialog(
      ...
      builder: (context) => _gamifiedDialog(
        ...
        content: isCorrect ? "Fantastic! You're really getting this! 🚀" : "The correct answer is:\n${widget.questions[_currentIndex].correctAnswer}",
  ...
  Widget build(BuildContext context) {
    return ThemedQuizScreen(
      title: 'Part XIV: Services Under Union Quiz',
